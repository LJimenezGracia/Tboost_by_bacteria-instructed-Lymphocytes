---
title: "Clustering & Annotation - Myeloid cells"
...
# Load Seurat object
seurat_obj <- readRDS(paste0(path_r_objects, "/VEIGAEST_myeloidcells_clustering_level3_annotation.rds"))
...
seurat_obj_integrated$annot_level4 <- seurat_obj_integrated@active.ident
Idents(seurat_obj_integrated) <- "annot_level4"
```
...
openxlsx::write.xlsx(biomarkers_list, file = paste0(path_r_tables, "/VEIGAEST_biomarkers_level4_myeloidcells_res0.4.xlsx"))
```
...
cDC1_genes <- c("Irf8", "Cd24a", "Batf3", "Xcr1", "H2-Ab1", "Clec9a")
...
```{r fig.width=14, fig.height=10}
cDC2_genes <- c("Mgl2", "Sirpa", "Itgam", "Irf4", "Clec10a",
                "Irf8", "Batf3", "Cd207", # negative for cDC1 markers
                "Cd163", "Lrp1", "Itgax", "Cd209d" # resident
                ) 
...
```{r fig.width=14, fig.height=10}
mregDC_genes <- c("Fscn1", "Cacnb3", "Ccr7", "Cd40", "Tmem123", "Cd274")

FeaturePlot(seurat_obj_integrated,
            features = mregDC_genes,
            #split.by = "tissue",
            pt.size = 0.25,
            ncol = 3
)
...
```{r fig.width=14, fig.height=10}
monomacro_genes <- c("Lyz2", "C1qb", "C1qa", "Apoe", "Selenop", # Macro
                     "Plac8", "Ly6c2", "Chil3", "Hp", # Mono
                     "Rsad2", "Irf7", "Ms4a4c", "Isg15", # Monomature
                     "Csf1r", "	Adgre1", "Itgam" #M0
                     )
...
https://pmc.ncbi.nlm.nih.gov/articles/PMC8268869/

```{r fig.width=14, fig.height=15}
M1macro_genes <- c("Ifitm1", "Thbs1", "Flrt3", "Il7r", "Il1b", "Ccr1",
                   "Marco", "Cxcl9", "Cxcl10", "Cxcl11", "Nos2", "Socs1" #M1
                   )
...
M2macro_genes <- c("Apoe", "Selenop", "C1qa", "C1qb", "Cd163", "Cd68", "Mrc1", "Cd86", "Adgre1", "Cd300c2",
                   "Cd206", "Tgm2", "Fizz1", "Chil3", "Arg1", "Ccl22", "Cd163", #M2
                   "Slamf9", "Cd72", "Itgam", "Adgre1", # Slamf9 macros
                   "Ccl8"
                   )
...
seurat_obj_integrated$annot_level4 <- as.character(seurat_obj_integrated@active.ident)

# Exploring number of celltypes
table(seurat_obj_integrated$annot_level4)
```
...
gg_umap_clustering_annot <- DimPlot(object = seurat_obj_integrated,
        group.by = "annot_level4",
        label = TRUE,
        label.size = 5,
        repel = TRUE,
        cols = color_palette
        ) +
  labs(title = "Myeloid cells - level4")
gg_umap_clustering_annot

ggsave(filename = paste0(path_r_figs, "/VEIGAEST_clustering_level4_myeloidcells_annotation_umap.png"),
...
```{r}
# Save annotations
saveRDS(seurat_obj_integrated@meta.data[c("annot_level1", "annot_level2", "annot_level3", "annot_level4")],
        paste0(path_r_objects, "/VEIGAEST_myeloidcells_annotation_level4.rds"))
```
...
seurat_obj_integrated$annot_level4 <- as.factor(as.character(seurat_obj_integrated$annot_level4))

table(seurat_obj_integrated$annot_level4)
length(unique(seurat_obj_integrated$annot_level4))
```


```{r}
# Save processed lognorm Seurat object
saveRDS(seurat_obj_integrated, paste0(path_r_objects, "/VEIGAEST_myeloidcells_clustering_level4_annotation.rds"))
#seurat_obj_integrated <- readRDS(paste0(path_r_objects, "/VEIGAEST_myeloidcells_clustering_level4_annotation.rds"))
```
