---
title: "Clustering & Annotation - Neutrophils"
...
# Load Seurat object
seurat_obj <- readRDS(paste0(path_r_objects, "/VEIGAEST_clustering_level1_neutrophils.rds"))
...
openxlsx::write.xlsx(biomarkers_list, file = paste0(path_r_tables, "/VEIGAEST_biomarkers_level2_neutrophils_res0.25.xlsx"))
```
...
mastcells_genes <- c("Mcpt4", "Gata2", "Ms4a2", "Kit", "Fcer1a")

FeaturePlot(seurat_obj_integrated,
            features = mastcells_genes,
            #split.by = "sample_metsize",
            #order = T,
            pt.size = 0.25,
            label = TRUE,
            repel = TRUE,
...
neutrophils_genes <- c("Cd14", "S100a8", "S100a9", "Slpi", "Csf3r", "G0s2", "Ly6g")

FeaturePlot(seurat_obj_integrated,
            features = neutrophils_genes,
            #split.by = "sample_metsize",
            #order = T,
            pt.size = 0.25,
            label = TRUE,
...
```{r fig.width=25, fig.height=10}
maturation_genes <- c("Eef1a1", # progenitors
                       "Elane", # early transition
                       "Ngp", # late transition
                       "Mxd1", "Mmp8", "Mmp9", "Retnlg", "Cd52" # mature neutrophils
                       )

FeaturePlot(seurat_obj_integrated,
            features = maturation_genes,
            #split.by = "sample_metsize",
            #order = T,
            pt.size = 0.25,
            label = TRUE,
...
Pro-tumor/anti-inflammatory (N2) and anti-tumor/pro-inflammatory (N1) neutrophils
 
```{r fig.width=20, fig.height=15}
N1_genes <- c("Ccr5", "Gnb1", "Stat2", "Jak2", "Jak3", "Rac1", "Rac2",
              "Raf-1", "Erk1", "Ekr2", "Ptk2", "Il13r", "Il15r", "Ifnb",
              "Ccl3", "Ccl7", "Cxcl4", "Cxcl9", "Cxcl10", "Ccl6"," Cxcl13", "Icam1")
...
```{r fig.width=20, fig.height=15}
N2_genes <- c("Cxcr1", "Cxcr2", "Il7r", "Il18r", "Tgfb1", "Tgfb2", "Tgfb3",
              "Ccl2", "Ccl3", "Ccl4", "Ccl8", "Ccl12", "Ccl17", "Cxcl14", "Cxcl1", "Cxcl2", "Cxcl8", "Il6", "Cxcl16", "Stat3", "Mmp9", "Ccl5")
...
seurat_obj_integrated <- RenameIdents(
 seurat_obj_integrated,
  "0" = "Neutrophils",
  "1" = "Neutrophils",
  "2" = "Doublets", # with Myeloid cells
  "3" = "Neutrophils",
  "4" = "Doublets", # with T cells
  "5" = "Mast cells"
)
...
gg_umap_clustering_annot <- DimPlot(object = seurat_obj_integrated,
  ...
  labs(title = "Neutrohils - level2")
gg_umap_clustering_annot

ggsave(filename = paste0(path_r_figs, "/VEIGAEST_clustering_level2_neutrophils_annotation_umap.png"),
...
saveRDS(seurat_obj_integrated@meta.data[c("annot_level1", "annot_level2")],
        paste0(path_r_objects, "/VEIGAEST_neutrophils_annotation_level2.rds"))
```

## Remove uninteresting cells
```{r}
Idents(seurat_obj_integrated) <- "annot_level2"
seurat_obj_integrated <- subset(seurat_obj_integrated, idents = c("Doublets"), invert = TRUE)
...
# Save processed Seurat objects
```{r}
saveRDS(seurat_obj_integrated, paste0(path_r_objects, "/VEIGAEST_neutrophils_clustering_level2_annotation.rds"))
#seurat_obj_integrated <- readRDS(paste0(path_r_objects, "/VEIGAEST_neutrophils_clustering_level2_annotation.rds"))
```

# Generate Seurat objects (by cell lineage)
```{r}
# T Lymphocytes
seurat_obj_neu <- subset(seurat_obj_integrated, idents = "Neutrophils")
saveRDS(seurat_obj_neu, paste0(path_r_objects, "/VEIGAEST_neutrophils_clustering_level2_annotation_neutrophils.rds"))
rm(seurat_obj_neu)
```
