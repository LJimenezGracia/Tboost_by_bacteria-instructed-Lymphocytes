...
path_r_results <- here::here("04_GEX_analysis/DEA_and_GSEA/results/DEA_celltypes")
...
for (index in 1:length(comparison_name_list)) {
  ...
  path_object <- paste0(path_r_results, "/VEIGAEST_DEgenes_celltypes_", comparison_name, ".rds")
  path_file <- paste0(path_r_results, "/VEIGAEST_DEgenes_celltypes_", comparison_name, ".xlsx")
  path_file_split <- paste0(path_r_results, "/VEIGAEST_DEgenes_celltypes_", comparison_name, "_UPDOWN.xlsx")
  ...
  seurat_obj_subset$cell_types <- as.factor(as.character(seurat_obj_subset$cell_types))
  
  DE_genes_bycell <- purrr::map(levels(seurat_obj_subset$cell_types), function(celltype) {
    seurat_obj_subset <- seurat_obj_subset[, seurat_obj_subset$cell_types == celltype]    
  ...
  names(DE_genes_bycell) <- levels(seurat_obj_subset$cell_types)
