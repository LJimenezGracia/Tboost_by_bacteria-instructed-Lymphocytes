...
def main():
    ...
    for subproject_folder in subproject_folder_list:
        subproject_folder_path = "{}/{}".format(subprojects_path, subproject_folder)
        if not os.path.exists(subproject_folder_path):
            os.mkdir(subproject_folder_path)

        mask = (metadata_df["subproject_folder"] == subproject_folder)
        ...
        fastq_df_path = "{}/fastq_paths.csv".format(subproject_folder_path)
        ...
        jobs_path = "{}/jobs".format(subproject_folder_path)
        ...
        for gem_id in gem_id_list:
            ...
            filt = ((metadata_df["subproject_folder"] == subproject_folder) & (metadata_df["gem_id"] == gem_id))
            ...
            if lib_hashing == "hashed_barcode":
                # Create feature_reference.csv: file indicating correspondence between HTO and sample id
                filt_row = ((feature_reference_df["subproject_folder"] == subproject_folder) & (feature_reference_df["gem_id"] == gem_id))
                filt_col = np.invert(feature_reference_df.columns.isin(["subproject_folder", "gem_id"]))
