...
## Load data
The data used in this Rmarkdown document comes from **VEIGAEST** dataset, filtered matrices were processed with `cellranger v9.0.0`, and the doublet predictions were obtained using `scrublet`.
```{r}
...
seurat_obj_list <- purrr::map(gem_id_list, function(gem_id) {
  path_filtered_matrix <- here::here(
    paste(path_cellranger_data, gem_id, gem_id, "outs/per_sample_outs", gem_id, "/count/sample_filtered_feature_bc_matrix", sep = "/"))
...
names(seurat_obj_list) <- gem_id_list
```

Here, adding the doublet predictions computed with Scrublet.
```{r}
# Add doublet predictions
scrublet_files_list <- purrr::map(gem_id_list, function(gem_id) {
  path_scrublet <- here::here(paste0("02_QC/results/doublet_prediction/tables/", gem_id, "_doublet_prediction_scrublet.csv"))
})

scrublet_dfs <- lapply(scrublet_files_list, function(scrublet_file_path) {
  read.csv(scrublet_file_path)
})
names(scrublet_dfs) <- gem_id_list

# Integrate scrublet doublet predictions to seurat metadata
seurat_obj_list <- purrr::map2(seurat_obj_list, names(seurat_obj_list),
...
seurat_obj$treatment <- as.factor(seurat_obj_metadata$treatment)
```
...
It is important to keep in mind that every library contains cells from a single patient, but for some of them we have multiplexed blood and synovial fluid together.
...
ggdens_libsize <- seurat_obj@meta.data %>%
  ggplot(aes(x = nCount_RNA, fill = library_name, color = library_name)) +
...
ggdens_ngenes <- seurat_obj@meta.data %>%
  ggplot(aes(x = nFeature_RNA, fill = library_name, color = library_name)) +
    geom_density(alpha = 0.2) + 
    scale_x_log10() +
...
ggdens_pctmt <- seurat_obj@meta.data %>%
  ggplot(aes(x = pct_mt, fill = library_name, color = library_name)) +
    geom_density(alpha = 0.2) + 
...
ggdens_pctrb <- seurat_obj@meta.data %>%
  ggplot(aes(x = pct_rb, fill = library_name, color = library_name)) +
    geom_density(alpha = 0.2) + 
...
```{r}
saveRDS(seurat_obj, paste0(path_r_objects, "/VEIGAEST_alldata_filtered.rds"))
```
